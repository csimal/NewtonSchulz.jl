...
The matrix sign of a rectangular matrix $A \in \R^{m \times n}$ is defined as
$$ \operatorname{msign}(A) = UV^\top = (A A^\top)^{-1/2} A = A (A^\top A)^{-1/2}, $$
where $A = U\Sigma V^\top$ is the (thin) SVD of $A$. In essence $\operatorname{msign}(A)$ is the matrix with the same singular vectors as $A$ but with all non-zero singular values set to one.

Using the SVD decomposition is the simplest method to compute $\operatorname{msign}$, but is computationally expensive, and not efficient on GPUs. An alternative approach consists in computing the following sequence
$$ X_{t+1} = a X_t + b (X_t X_t^\top) X_t + c (X_t X_t^\top)^2 X_t, $$
where $X_0 = A / \|A\|_F$, and $(a,b,c)=(2,-1.5,0.5)$. Each iteration preserves the singular vectors while applying the quintic polynomial $p(x)=ax + bx^3 + cx^5$ to the singular values. It can be shown that $\lim_{n\rightarrow \infty}p^{(n)}(x) = \operatorname{sign}(x)$, and so that $\lim_{t\rightarrow \infty} X_t = \operatorname{msign}(A)$. The advantage of this approach is that each iteration only involves matrix multiplications and additions, so it can be performed efficiently on GPU. 
...
$$ \operatorname{mcsgn}(A) = (A^2)^{-1/2} A = A (A^2)^{-1/2}. $$
This latter function is invariant under similarity transformation ($\operatorname{mcsgn}(PAP^{-1} = \operatorname{mcsgn}(A)$), and if $A$ is diagonalizable, $\operatorname{mcsgn}(A)$ is the matrix with the same eigenvectors as $A$ and eigenvalues replaced by their sign.
